<?php
	...
	class Order{
		...
		function createOrder($data){
			$code="ONLINE-".time();
			$query="INSERT INTO orders(customer_id,address_receive,phone_receive,order_code,created_at,status,delivery) VALUES('".$data['customer_id']."','".$data['address_receive']."','".$data['phone_receive']."','".$code."','".Date('Y-m-d H:i:s')."','0','".$data['delivery'].")";
			$stmt= sqlsrv_query($this->conn, $query);
			echo sqlsrv_rows_affected($stmt);
			if (sqlsrv_rows_affected($stmt)>0) {
				foreach ($_SESSION['cart'] as $key => $value) {
					$sql="SELECT product_detail.id as productdetail_id,price FROM product_details as product_detail ,products WHERE product_detail.product_id= products.product_id AND color_id =".$value['color_id']." AND size_id= ".$value['size_id'];
					
					$sql1=sqlsrv_query($this->conn,$sql);
					$arr=sqlsrv_fetch_array($sql1, SQLSRV_FETCH_ASSOC);
					var_dump($arr);
					$sql2="INSERT INTOorder_details(product_detail_id,quantity_buy,price,order_code,created_at) VALUES(".$arr['productdetail_id'].",".$value['quantity_buy'].",".$arr['price'].",'".$code."','".Date('Y-m-d H:i:s')."')";
					echo $sql2;
